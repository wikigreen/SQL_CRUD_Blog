...
public class UserView implements View {
    ...
    public void create() {
        ...
        while(true){
            System.out.print("Type 'new' to add new post, or 'existing <id>' to add post from repository.\n" +
            ...
            if("new".equals(command)){
                ...
                Post post = new Post(null, content.toString());
                posts.add(post);
                continue;
            }
    ...
    public void update(Long id) {
        ...
        Role role = null;
        while(true){
            System.out.println("Type role (user, moderator or admin). Press 'Enter' to skip:");
            String strRole = SCANNER.nextLine().trim().toUpperCase();
            if("".equals(strRole))
                break;
            try{
                role = Role.parseRole(strRole);
